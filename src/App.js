...
function Hole(props){
  let v =''
  if (props.value === 'Red') v = 'X'
  if (props.value === 'Blue') v = 'O'
  if (props.value === null) v = ''

  return (<div data-key={props['data-key']} className="Hole">
    <div className={props.value} >{v}</div></div>);
}
...
function Slat(props){
    ...
    return <div className="Slat" onClick={() => props.handleClick()}>
      {[...Array(props.holes.length)].map((x, j) => 
        <Hole data-key={j} value={props.holes[j]}></Hole>)}
      </div>
 }
...
class Board extends Component {
  ...
  makeMove(slatID){
    ...
    if( boardCopy[slatID].indexOf(null) !== -1 ){
      ...
      let newSlat = boardCopy[slatID].reverse();
      newSlat.shift();
      newSlat[5]=this.state.playerTurn
      newSlat.reverse()
      
    }
    
    const turn = (this.state.playerTurn === 'Red') ? 'Blue' : 'Red'
  ...
  aiMove() {
    if (this.state.gameMode === 'ai' && this.state.playerTurn === 'Blue') {
      ...
      setTimeout(function () { this.makeMove(validMove); }.bind(this), 700);
    ...
    return (
      <div>
        ...
        {this.state.gameSelected &&
          <div>
          <div>{this.state.playerTurn === 'Red'? 'My Turn: Red':'Your Turn:Blue'}</div>
          <div className="Board">
            {slats}
          </div></div>
        }
